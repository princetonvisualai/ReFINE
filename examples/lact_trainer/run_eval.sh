...
MODEL="ttt_baseline" # baseline,sft_baseline, ttt_baseline, 
TTT_REWARD="cosine_similarity" # cosine_similarity, binary
################################
...
if [ $DATASET_NAME == "longbench" ]; then
    ...
    if [ $TTT_RL == 1 ]; then
        ...
        task_micro_batch_size_per_gpu=1
        task_rollout_micro_batch_size_per_gpu=1
    fi
elif [ $DATASET_NAME == "ruler" ]; then
    val_batch_size=32
