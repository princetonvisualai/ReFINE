...
#SBATCH --gres=gpu:l40:4          # gpu
#SBATCH --cpus-per-task=32     # Specify the number of CPUs your task will need.
#SBATCH --mem=128G             # memory
#SBATCH -o /n/fs/wh-vlm/comp_vlm/slurm/%j_%x_%a.out # send stdout to outfile
#SBATCH -e /n/fs/wh-vlm/comp_vlm/slurm/%j_%x_%a.err # send stderr to errfile
#SBATCH -t 02:00:00           # time requested in hour:minute:second
#SBATCH --array=0-8         # specify the number of jobs to run
...
VAL_BEFORE_TRAIN=1
REWARD="cosine_similarity" # hybrid, binary, cosine_similarity
################################
...
elif [ $MODEL == "ttt_baseline_test" ]; then
    model_name="lact"
    model_path="/n/fs/xw-hh/lact_llm/lact-ttt-baseline-ruler-4096-sft-100"
...
# data files
train_files="/n/fs/xw-hh/tttrl_data/data/ruler/ruler_${dataset}_${model_name}_${max_seq_length}_4096_train.parquet"
val_files="/n/fs/xw-hh/tttrl_data/data/ruler/ruler_${dataset}_${model_name}_${max_seq_length}_500_test.parquet"
...
ttt_rollout_micro_batch_size_per_gpu=4
ttt_reward=$REWARD
ttt_k=5
ttt_n=1
ttt_temperature=1.0
...
experiment_name="${MODEL}_ruler_${dataset}_${max_seq_length}_${max_response_length}${ttt_tag}${task_tag}" 
